...
func saveFrame(frameNumber: Int) {
  ...
  do {
    ...
    print("Saved frame \(frameNumber) to \(fileName)")
  } catch {
    print("Failed to save frame \(frameNumber): \(error)")
  }
}

// Run for a few frames and save images
print("Starting upscaling test...")
application.run {
  modifyAtoms()
  modifyCamera()
  ...
  if frameID < 5 {  // Save first 5 frames
    print("Saving frame \(frameID)")
    saveFrame(frameNumber: frameID)
  }
